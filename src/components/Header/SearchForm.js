...
const navigate = useNavigate();
  ...
  return (
    <div ref={props.searchref} className={`${Style.searchForm} ${
        props.search ? `${Style.active}` : `${Style.inactive}`
      }`}>
        <div className={Style.searchFormContainer}>
            <h2>Search your posts</h2>
            <form onSubmit={handleSubmit}>
                <input type="text" name="search" id="search" placeholder='search your posts' value={search} onChange={(e) => setSearch(e.target.value)}/>
            </form>
        </div>
    </div>
  )
